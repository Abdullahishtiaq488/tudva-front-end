...
const CourseCard = ({
  ...
  const {
    ...
    instructor = {},
    level = 'All Levels',
    badge = { text: level, class: getBadgeClassForLevel(level) },
    totalLectures = 12
  } = course;

  // Get instructor data with fallbacks
  const instructorName = instructor?.fullName || instructor?.name || 'Instructor';
  const instructorRole = instructor?.role || 'Instructor';
  const instructorAvatar = instructor?.profilePicture || '/assets/images/avatar/01.jpg';

  // Calculate course duration in months (just for display)
  const courseDuration = Math.ceil(totalLectures / 4); // Roughly 4 lectures per month

  return <Card className="shadow-hover overflow-hidden bg-transparent">
    <div className="position-relative">
      <Image className="card-img-top" src={image} width={500} height={300} alt="Course image" />
        ...
        <div className="w-100 mt-auto d-inline-flex">
          <div className="d-flex align-items-center bg-white p-2 rounded-2">
            <div className="avatar avatar-sm me-2">
              <Image className="avatar-img rounded-1" src={instructorAvatar} width={40} height={40} alt="avatar" />
            </div>
            <div>
              <h6 className="mb-0"><Link href={`/course/${id}`} className="text-dark">{instructorName}</Link></h6>
              <span className="small">{instructorRole}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <CardBody>
      ...
      <ul className="list-inline">
        ...
        <li className="list-inline-item ms-2 text-reset">({course.reviewCount || 0})</li>
      </ul>
      <hr />
