...
const Instructor = ({ course }) => {
  // Get instructor from mock data
  console.log('Course in Instructor component:', course);
  console.log('Instructor ID:', course?.instructor_id);

  // First try to get instructor from course.instructor
  let instructor = course?.instructor;

  // If not found, try to find by ID
  if (!instructor && course?.instructor_id) {
    instructor = users.find(user => user.id === course.instructor_id);
    console.log('Found instructor by ID:', instructor);
  }

  // If still not found, use any instructor
  if (!instructor) {
    instructor = users.find(user => user.role === 'instructor');
    console.log('Using fallback instructor:', instructor);
  }
  return <>
    <Card className="mb-0 mb-md-4">
      <Row className="g-0 align-items-center">
        <Col md={5}>
          <Image src={instructor?.profilePicture} className="img-fluid rounded-3" alt="instructor-image" width={500} height={500} />
        </Col>
        <Col md={7}>
          <CardBody>
            <h3 className="card-title mb-0">{instructor?.fullName || instructor?.name || 'Instructor'}</h3>
            <p className="mb-2">Instructor of {course?.category || 'Course'}</p>
