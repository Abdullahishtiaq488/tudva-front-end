        ...
        {allReviews?.map((review, idx) => <Fragment key={idx}>
              ...
              <div>
                <div className="d-sm-flex mt-1 mt-md-0 align-items-center">
                  ...
                  {review.rating && <ul className="list-inline mb-0">
                      {Array(Math.floor(review.rating)).fill(0).map((_star, idx) => <li key={idx} className="list-inline-item me-1 small"><FaStar size={14} className="text-warning" /></li>)}
                      {!Number.isInteger(review.rating) && <li className="list-inline-item me-1 small"> <FaStarHalfAlt size={14} className="text-warning" /> </li>}
                      {review.rating < 5 && Array(5 - Math.ceil(review.rating)).fill(0).map((_star, idx) => <li key={idx} className="list-inline-item me-1 small"><FaRegStar size={14} className="text-warning" /></li>)}
                    </ul>}
                </div>
                ...
                <div className="btn-group" role="group" aria-label="Basic radio toggle button group">
                  ...
                  <label className="btn btn-outline-light btn-sm mb-0" htmlFor={`btnradio${idx}`}><FaRegThumbsUp className="me-1" />25</label>
                  <input type="radio" className="btn-check" name={`btnradi${idx}`} id={`btnradio${idx + 2}`} />
                  <label className="btn btn-outline-light btn-sm mb-0" htmlFor={`btnradio${idx + 2}`}> <FaRegThumbsDown className="me-1" />2</label>
                </div>
              </div>
            </div>
            {review.reply && review.reply.map((reply, idx) => <div className="d-md-flex mb-4 ps-4 ps-md-5" key={idx}>
                  <div className="avatar avatar-lg me-4 flex-shrink-0">
                    <Image className="avatar-img rounded-circle" src={reply.avatar} alt="avatar" />
                  </div>
                  <div>
                    <div className="d-sm-flex mt-1 mt-md-0 align-items-center">
