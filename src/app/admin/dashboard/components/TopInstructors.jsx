  ...
  return <Col lg={6} xxl={4}>
    <Card className="shadow h-100">
      ...
      <CardBody className="p-4">
        {users.filter(user => user.role === 'instructor').slice(0, 5).map((instructor, idx) => {
          ...
          return <React.Fragment key={instructor.id}>
            <TopInstructorsCard {...instructorData} />
            {users.filter(user => user.role === 'instructor').slice(0, 5).length - 1 !== idx && <hr />}
          </React.Fragment>;
        })}
      </CardBody>
    </Card>
